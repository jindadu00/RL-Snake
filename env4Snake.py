...
class Snake:
    ...
    def mystep(self, action):
        ...
        #   对于每条蛇
        #       道具时间减少1
        #       将action分解为单步，蛇瞬移过去，装上重叠就好，头部和尾部要分别保存
        #   撞击判定
        #       蛇与自己
        #       不同蛇之间
        #       蛇与墙壁之间
    ...
    def getState(self):
        ...
        for i, point in enumerate(checkPoint):
            if point[0] >= self.map_width or point[0] < 0 or point[
                    1] >= self.map_height or point[1] < 0:
                tem[i] = 1
        state = [deltax, deltay]
        state.extend(tem)
    ...
    def snake_is_alive(self, snake_coords):
        ...
        for snake_body in snake_coords[1:]:
            if snake_body['x'] == snake_coords[self.HEAD]['x'] and snake_body[
                    'y'] == snake_coords[self.HEAD]['y']:
                tag = False  # 蛇碰到自己身体啦
        return tag

    # 判断贪吃蛇是否吃到食物
    def snake_is_eat_food(self, snake_coords,
                          food):  # 如果是列表或字典，那么函数内修改参数内容，就会影响到函数体外的对象。
        flag = False
        if snake_coords[self.HEAD]['x'] == food['x'] and snake_coords[
                self.HEAD]['y'] == food['y']:
            while True:
