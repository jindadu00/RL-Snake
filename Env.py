...
class Snake:
    ...
    def __init__(self, id, init_body):
        self.isDead = False
...
def gen_head_matrix(snakes, current_map):
    ...
    for i, snake in enumerate(snakes):
        ...
        for cube in snake.body:
            if cube.isHead:
                head_matrix[cube.coordinates[0], cube.coordinates[1]] = 1
...
def update_body(snakes, snakes_matrix):
    for i, snake in enumerate(snakes):
        ...
        snake.body = [
            ...
            if snake_matrix[cell.coordinates[0], cell.coordinates[0]]
        ]
...
def detect_collision(current_map, snakes):
    ...
    for i, snake in enumerate(snakes):
        ...
        snake_i_head = np.repeat(head_maxtrix[:, :, i:i + 1],
                                 ...
                                 axis=2)
        hh_idxs = np.unique(
            np.where((snake_i_head == head_maxtrix) & (snake_i_head == 1))[2])
        # judge head hit tail
        ht_idxs = np.unique(
...
if __name__ == "__main__":
    ...
    r = current_map.row
    snake_list = [
        Snake(1, np.array([round(r / 3), round(c / 4)])),
        Snake(2, np.array([round(r / 3), round(2 * c / 4)])),
        Snake(3, np.array([round(r / 3), round(3 * c / 4)])),
        Snake(4, np.array([round(2 * r / 3), round(c / 4)])),
